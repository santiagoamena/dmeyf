...
carpeta_datasetsOri <-  "C:/Users/santi/projects/maestria/dmef/datasets_ori/"
...
param_fe <- list(objective = "binary:logistic",
                scale_pos_weight=160.201198630137,
                eta = 0.09723526495894,
                max_depth = 3,
                min_child_weight=6,
                gamma=232,
                max_bin=15)
...
xgb_create_features <- function(model, data) {
  pred_with_leaf <- predict(bst, data, predleaf = TRUE)
  cols           <- lapply(as.data.frame(pred_with_leaf), factor)
  cbind(data, cols)
}

df = data.matrix(ds)
pred_with_leaf <- predict(bst, df, predleaf = TRUE)
cols           <- lapply(as.data.frame(pred_with_leaf), factor)
cols_df = as.data.frame(cols)
new_features_train = cbind(ds, cols_df)
